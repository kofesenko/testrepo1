...
resource "aws_subnet" "public_subnet_az" {
  count = length(var.public_subnet_cidr_az)

  cidr_block        = element(var.public_subnet_cidr_az, count.index)
...
resource "aws_route_table_association" "public_subnet_az" {
  ...
  subnet_id      = element(aws_subnet.public_subnet_az[*].id, count.index)
  route_table_id = aws_route_table.public_subnet.id
}